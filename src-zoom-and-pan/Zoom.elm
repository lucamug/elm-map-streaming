...
type Zoom
    = Zoom
        ...
        , positionHistoryForInertia : Maybe { last : ( Float, Float ), beforeLast : ( Float, Float ) }
        , latestMouseDownOffset : Maybe ( Float, Float )
        }


isDragging : Zoom -> Bool
isDragging (Zoom zoom) =
    case zoom.drag of
        Just _ ->
            True
...
type OnZoom
    ...
    | MouseDown ( Float, Float ) (Maybe Matrix2x3) ( Float, Float )
...
init { width, height } =
    Zoom
        ...
        , transition = Nothing
        , transitionInertia = Nothing
        , positionHistoryForInertia = Nothing
        , latestMouseDownOffset = Nothing
        }
...
onDrag (Zoom { drag }) tagger =
    case drag of
        Nothing ->
            [ custom "mousedown"
                (D.map5
                    (\x y matrix offsetX offsetY ->
                        { message = tagger (MouseDown ( x, y ) matrix ( offsetX, offsetY ))
                        , stopPropagation = True
...
subscriptions (Zoom zoom) tagger =
    Sub.batch
            ...
            Just _ ->
                Sub.batch
                    ...
                    , Browser.Events.onMouseUp
                        (D.map2
                            (\offsetX offsetY ->
                                tagger (MouseUp ( offsetX, offsetY ))
                            )
                            (D.field "offsetX" D.float)
                            (D.field "offsetY" D.float)
                        )
        ...
        -- INERTIA
        , case zoom.transition of
            Just transition ->
                -- If transition, we keep ticking
                Browser.Events.onAnimationFrameDelta (Tick >> tagger)

            Nothing ->
                Sub.none
        , case zoom.transitionInertia of
            Just transition ->
                -- If transition, we keep ticking
                Browser.Events.onAnimationFrameDelta (TickInertia >> tagger)
    ...
    case msg of
            ...
            case model.drag of
                Just drag ->
                    let
                        ...
                        current : ( Float, Float )
                        current =
                            Transform.invert position trasform_
                    in
                    Zoom
                        { model
                            | drag = Just { drag | current = current }
                            , transform = trasform_ |> constrain model.extent model.translateExtent
                            , transition = Nothing
                            , transitionInertia = Nothing
                            , positionHistoryForInertia =
                                case model.positionHistoryForInertia of
                                    Just positionHistoryForInertia ->
                                        Just { last = position_, beforeLast = positionHistoryForInertia.last }

                                    Nothing ->
                                        Just { last = position_, beforeLast = position_ }
                        }
                            ...
                            Just after ->
                                { model
                                    ...
                                    , positionHistoryForInertia =
                                        case model.positionHistoryForInertia of
                                            Just positionHistoryForInertia ->
                                                Just { last = after.position, beforeLast = positionHistoryForInertia.last }

                                            Nothing ->
                                                Just { last = after.position, beforeLast = after.position }
                                }
        ...
        TouchEnded touches ->
            Zoom
                ...
                |> (\(Zoom m) ->
                        --
                        -- Not sure if this extra check for the assciated
                        -- touch is needed or not. It is done also above so I just
                        -- copied. Maybe we should bring them together.
                        --
                        case model.touches of
                            OneFinger touch1 ->
                                case findAssociatedTouch touch1 touches of
                                    ...
                                    Nothing ->
                                        Zoom { m | transitionInertia = Nothing }
        ...
        TickInertia delta ->
            case model.transitionInertia of
                Just transitionInertia ->
                    let
                        t : Transform
                        t =
                            model.transform
                        ...
                        easing =
                            easingOutCubic (clamp 0 1 ((transitionInertia.elapsed + delta) / transitionInertia.duration))
                    in
                    Zoom
                        { model
                            | transform =
                                { t
                                    | x = transitionInertia.lastTransform.x + (transitionInertia.d.dx * easing)
                                    , y = transitionInertia.lastTransform.y + (transitionInertia.d.dy * easing)
                                }
                            , transitionInertia =
                                if transitionInertia.elapsed + delta >= transitionInertia.duration then
                                    -- Transition ended
                                    Nothing

                                else
                                    Just { transitionInertia | elapsed = transitionInertia.elapsed + delta }
                        }
...
isClick : OnZoom -> Zoom -> Maybe ( Float, Float )
isClick msg (Zoom model) =
    case msg of
        MouseUp offset ->
            if Just offset == model.latestMouseDownOffset then
                -- The user didn't move between a modedown and mouseup so we
                -- conside this a click
                Just offset
...
startInertia : Zoom -> Zoom
startInertia (Zoom model) =
    case model.positionHistoryForInertia of
        Nothing ->
            Zoom { model | positionHistoryForInertia = Nothing }

        Just positionHistory ->
            let
                ( lastX, lastY ) =
                    positionHistory.last

                ( beforeLastX, beforeLastY ) =
                    positionHistory.beforeLast

                delta : { dx : Float, dy : Float }
                delta =
                    { dx = (lastX - beforeLastX) * 20, dy = (lastY - beforeLastY) * 20 }

                combinedDelta : Float
                combinedDelta =
                    abs delta.dx + abs delta.dy
            in
            if combinedDelta < 30 then
                -- Too small, no need to start inertia
                Zoom { model | positionHistoryForInertia = Nothing }
...
easingOutCubic : Float -> Float
easingOutCubic t =
    1 - ((1 - t) ^ 3)


easingInCubic : Float -> Float
easingInCubic t =
    t ^ 3
...
setTransform transition trfm (Zoom model) =
    ...
    case transition of
        ...
        Instantly ->
            Zoom { model | transform = internalTransform, transition = Nothing, transitionInertia = Nothing }
